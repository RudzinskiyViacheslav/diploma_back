class ProductionPointHandlers {
  ...
  PSQLSearchByCapfkid =
    "SELECT * FROM departments WHERE department_capfk_id=$1";
  ...
  handler = (request, response) => {
    if (request.query.capfk_id)
      this.searchByCapfkId(request, response, request.query.capfk_id);
      ...
      (result) => {
        ...
        if (result.length !== 0) response.status(200).json(result);
          ...
          response
            .status(404)
            .json(
              `Производственный участок филиала с id ${capfk_id} не найден`
            );
      },
      (error) => {
        response.status(500);
    ...
    this.searchProdPointByDepartmentId(this.psqlPool, department_id).then(
      (result) => {
        ...
        if (result.length !== 0) response.status(200).json(result);
          ...
          response
            .status(404)
            .json(`Производственный участок с id ${department_id} не найден`);
      },
      (error) => {
        response.status(500);
        throw error;
